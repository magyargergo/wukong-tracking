...
function ensureAdminByUsername(username: string): boolean {
  const dbUser = getUserByUsername(username);
  return (dbUser?.is_admin ?? 0) === 1;
}
function ensureAdminBySession(token?: string): boolean {
  if (!token) return false;
  const s = getSessionByToken(token);
  if (!s || s.revoked === 1) return false;
...
export async function GET(req: Request) {
  ...
  if (!ensureAdminBySession(token)) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
...
export async function POST(req: Request) {
  ...
  const id = upsertUser({ username, name, password_hash, is_admin });
...
export async function PUT(req: Request) {
  ...
  upsertUser({ id, username, name, password_hash, is_admin });
